...
def quantize_to_int(x, scale, dtype=np.int32):
    """
    Quantize float tensor to integer using round-to-nearest.
    Example: Q7 scale = 128.
    """
...
def requantize_round(x_int, divisor):
    """
    Requantize integer accumulator back to activation scale.
    This avoids floor division error from //.
    """
...
# =========================================================
# Dump float model weights to fixed-point txt/bin
# =========================================================
...
def dump_float_weights_to_txt_and_bin(
    ...
    """
    Dump Dense layer weights/biases to integer files.

    Important:
    - weight_int = round(weight_float * weight_scale)
    - bias_int   = round(bias_float * input_scale * weight_scale)

    Because Dense computes:
        y_float = x_float @ w_float + b_float

    If:
        x_int = x_float * input_scale
        w_int = w_float * weight_scale

    then:
        acc_int = x_int @ w_int + b_int
        y_int   = acc_int / weight_scale

    so b_int must use input_scale * weight_scale.
    """
    ensure_dir(save_dir)
...
class Utils:
    ...
    def load_data(self):
        ...
        # IMPORTANT:
        # Use train mean/std for both train and test.
        # Do not normalize test by its own mean/std.
    ...
    def check_float_manual_inference(self):
        """
        Compare Keras float inference with manual Dense computation.
        """
        ...
        for i in range(len(self.x_test)):
            ...
            for layer_idx in range(4):
                ...
                # ReLU for layer 0, 1, 2 only.
                # Last layer is logits before softmax.
                if layer_idx < 3:
    ...
    def infer_one_int(self, x_float):
        """
        Integer inference simulation.

        data_int uses activation scale = 128.
        Each Dense layer:
            acc = data_int @ weight_int + bias_int
            data_int = round(acc / weight_scale)

        Because:
            data_int scale = activation_scale
            weight_int scale = weight_scale
            bias_int scale = activation_scale * weight_scale
            acc scale = activation_scale * weight_scale
            after divide by weight_scale -> activation_scale
        """
    ...
    def write_model_h(self):
        ...
        # Bias should stay int32 because bias_scale = input_scale * weight_scale.
    ...
    # =====================================================
    # Optional: Vitis AI quantization flow
    # =====================================================
...
if __name__ == "__main__":
    ...
    # Step 1: train or load float model
    model = utils_obj.train_or_load_float_model(train=False)

    # Step 2: verify manual float inference
    utils_obj.check_float_manual_inference()

    # Step 3: dump fixed-point weights from float model
    utils_obj.dump_quant_params_from_float()

    # Step 4: load fixed-point weights
    utils_obj.load_quant_params()

    # Step 5: evaluate integer inference
    utils_obj.evaluate_int()

    # Step 6: export C header
